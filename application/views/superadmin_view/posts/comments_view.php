...
<div class="col-sm-10">
 <?php $si=$this->uri->segment(4);?>
    <form method="post" action="<?php echo HTTP_BASE_PATH;?>admin/posts/search_comments/<?php echo $si;?>" >
        <div class="col-sm-12">
    <div class="row m-t-15" style="margin-right: 0px;">


  <div class="bg-site col-wh col-sm-12 head-pad hgh-40"> 
    <h5 class="m-t-n6" style="margin-bottom: 0px;">Comments</h5>    
        </div>
    ...
    <div class="input-group col-sm-4">
      <input type="text" class="form-control no-bod-rad" placeholder="Search by commented By" name="search_str" id="search_str">
        ...
        <table class="table table-striped table-bordered">
			<thead>
			  <tr>
				  ...
				  <th style="text-align: center;vertical-align: middle">Commented By</th>
				  <th style="text-align: center;vertical-align: middle">Replies</th>
				  <th style="text-align: center;vertical-align: middle">Commented On</th>
					...
					foreach($row as $records){
						...
						<tr>
							<td><div class="form-check m-t-n15">
							    <label class="form-check-label">
								    <input type="checkbox" class="form-check-input cnames" name="cnames[]" value="<?php echo $records->comment_id;?>" >
									</label>
								</div></td>
							<td><?php echo $c;?></td>
							<td><?php echo $records->comment; ?></td>
							<td><?php echo $records->name; ?></td>
							<td><a href="<?php echo base_url(); ?>admin/posts/comments_replies/<?php echo $records->comment_id; ?>">click here</a></td>
							<td><?php $v=$records->commented_on; $v=explode(" ",$v);$d=$v[0];$d=explode("-",$d);$r=array_reverse($d);$cd=implode('/',$r);
							echo $cd;?></td>
							<td><?php $s=$records->comment_status;
				   			if($s==1){
				   				?>
				   				<a  href="<?php echo base_url();?>admin/posts/update_comment/<?php echo $records->comment_id; ?>/<?php echo $records->comment_status; ?>/<?php echo $si;?>" class="badge badge-success">Active</a>
				   				<?php
				   			}
				   			else{
				   				?>
				   				<a  href="<?php echo base_url();?>admin/posts/update_comment/<?php echo $records->comment_id;?>/<?php echo $records->comment_status;?>/<?php echo $si;?>" class="badge badge-danger">Inactive</a>
				   				<?php
				   			}
				   			?></td>
							<td>
							</a>&nbsp;<a title="Delete" onclick="javascript:return confirm('Are you sure to delete');" href="<?php echo base_url();?>admin/posts/delete_comment/<?php echo $records->comment_id;?>/<?php echo $si;?>"><i class="fa fa-trash text-danger" aria-hidden="true" ></i></a> &nbsp;</td>
        				</tr>
						<?php
						$c=$c+1;
					}
