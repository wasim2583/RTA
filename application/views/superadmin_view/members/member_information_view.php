...
<div class="col-sm-10">
 <?php $si=$this->uri->segment(4);?>
    <form method="post" action="<?php echo HTTP_BASE_PATH;?>admin/members/search/<?php echo $si;?>" >
        <div class="col-sm-12">
    <div class="row m-t-15" style="margin-right: 0px;">


  <div class="bg-site col-wh col-sm-12 head-pad hgh-40"> 
    <h5 class="m-t-n6" style="margin-bottom: 0px;">Group Members Details<?php if(isset($ac)) echo "hi";?></h5>    
        </div>
    ...
    <div class="input-group col-sm-4">
      <input type="text" class="form-control no-bod-rad" placeholder="Search by group name" name="search_str" id="search_str">
			   ...
			   if(count($row)>0){
			   	...
			   	foreach($row as $rec){
			   		...
			   		<tr>
			   			...
			   			<td style="text-align:center"><?php echo $rec['total']?></td>
			   			<td style="text-align:center"><?php $v=$rec['added_on'];$v=explode(" ",$v);$d=$v[0];$d=explode("-",$d);$r=array_reverse($d);$cd=implode('/',$r);
			   			echo $cd;
			   			?></td>
			   			<td style="text-align:center">
			   			</a>&nbsp;<a title="Delete" onclick="javascript:return confirm('Are you sure to delete');" href="<?php echo base_url();?>admin/members/delete_member/<?php echo $rec['member_id'];?>/<?php echo $si;?>"><i class="fa fa-trash text-danger" aria-hidden="true" ></i></a> &nbsp;</td>
			   		</tr>
			   		<?php
			   		$c=$c+1;
			   	}
