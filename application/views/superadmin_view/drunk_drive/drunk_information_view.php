...
<div class="col-sm-10">
 <?php $si=$this->uri->segment(4);?>
  ...
  <div class="bg-site col-wh col-sm-12 head-pad hgh-40"> 
    <h5 class="m-t-n6" style="margin-bottom: 0px;">Drun & drive Information<?php if(isset($ac)) echo "hi";?></h5>    
        </div>
...
<?php  $success=$this->session->flashdata('success');
 ...
 <table class="table table-striped table-bordered">
	...
	<th>S.No</th>
      ...
      <label class="form-check-label">
        <input type="checkbox" class="form-check-input cnames" name="cnames[]" value="<?php echo $rec['drunk_case_id'];?>" >
      </label>
    </div></td>     <td><?php echo $c;?></td>
					<td><p><?php echo ucfirst($rec['location']);?></p></td>
                    <td><p><?php $date=date_create($rec['date']); echo date_format($date,'d-m-Y');?></p></td>
					<td><p><?php echo ($rec['from_hrs']);?></p></td>
					<td><p><?php echo ($rec['to_hrs']);?></p></td>
					
					<td><p><?php echo ucfirst($rec['city']);?></p></td>
					
                    <td><a title="Update" href="<?php echo base_url();?>admin/headings/update/<?php echo $rec['drunk_case_id'];?>/<?php echo $si;?>"><i class="fa fa-pencil-square-o text-primary" aria-hidden="true" ></i>
					</a>&nbsp;<a title="Delete" onclick="javascript:return confirm('Are you sure to delete');" href="<?php echo base_url();?>superadmin/heading_controller/delete_title/<?php echo $rec['drunk_case_id'];?>/<?php echo $si;?>"><i class="fa fa-trash text-danger" aria-hidden="true" ></i></a> &nbsp;<a title="Generate PDF"  href="<?php echo base_url();?>Pdf_controller/pdf_create/<?php echo $rec['drunk_case_id']; ?>">PDF</a></td>
