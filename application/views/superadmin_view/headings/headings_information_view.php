...
<?php  $success=$this->session->flashdata('success');
 ...
 <table class="table table-striped table-bordered">
	...
	<th>S.No</th>
      ...
      <label class="form-check-label">
        <input type="checkbox" class="form-check-input cnames" name="cnames[]" value="<?php echo $rec['heading_id'];?>" >
      </label>
    </div></td>     <td><?php echo $c;?></td>
                    <td><p><?php echo ucfirst($rec['heading_title']);?></p></td>
					<td><p><?php echo ucfirst($rec['heading_description']);?></p></td>
					<?php $date=date_create($rec['added_on']);?>
					 <td><?php echo date_format($date,'d-m-Y H:i:s');?></td>
                    <td><?php $s=$rec['status'];
					if($s==1){ ?>
					<a  href="<?php echo base_url();?>superadmin/heading_controller/update_status/<?php echo $rec['heading_id'];?>/<?php echo $rec['status'];?>/<?php echo $si;?>" class="badge badge-success">Active</a> 
					<?php }else{?>
					<a  href="<?php echo base_url();?>superadmin/heading_controller/update_status/<?php echo $rec['heading_id'];?>/<?php echo $rec['status'];?>/<?php echo $si;?>" class="badge badge-danger">Inactive</a>
					<?php } ?></td> 
                    <td><a title="Update" href="<?php echo base_url();?>admin/headings/update/<?php echo $rec['heading_id'];?>/<?php echo $si;?>"><i class="fa fa-pencil-square-o text-primary" aria-hidden="true" ></i>
					</a>&nbsp;<a title="Delete" onclick="javascript:return confirm('Are you sure to delete');" href="<?php echo base_url();?>superadmin/heading_controller/delete_title/<?php echo $rec['heading_id'];?>/<?php echo $si;?>"><i class="fa fa-trash text-danger" aria-hidden="true" ></i></a> &nbsp;</td>
