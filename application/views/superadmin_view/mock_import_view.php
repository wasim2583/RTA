...
<div class="col-sm-10">
 <?php //$si=$this->uri->segment(4);?>
    <form method="post" action="<?php echo HTTP_BASE_PATH;?>superadmin/Imp_mocktest_controller/uploadData" enctype="multipart/form-data" >
        <div class="col-sm-12">
 ...
 <button type="submit" class="btn btn-primary btn-sm no-bod-rad pull-right" name="managemock" style="margin-top: -20px;"><i class="fa fa-dot-circle-o" aria-hidden="true"></i> Manage Mocktest</button>		
        </div>
<?php 	 $import=$this->session->flashdata('import');
		 ...
		 <?php 	 $hospital_file=$this->session->flashdata('hospital_file');
		 if($hospital_file){?>      
		 <div class="alert alert-danger div3 col-sm-12">
         <strong></strong><?php echo $hospital_file;?>
		 </div> <?php }?> 
		 
		 
		 <?php 	 $no_import=$this->session->flashdata('no_import');
		 if($no_import){?>      
		 <div class="alert alert-danger div3 col-sm-12">
         <strong></strong><?php echo $no_import;?>
		 </div> <?php }?> 
