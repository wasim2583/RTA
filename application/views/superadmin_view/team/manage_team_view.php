...
<div class="col-sm-10">
 <?php $si=$this->uri->segment(4);?>
    <form method="post" action="<?php echo HTTP_BASE_PATH;?>superadmin/team_ctrl/search/<?php echo $si;?>" >
        <div class="col-sm-12">
    <div class="row m-t-15" style="margin-right: 0px;">


  <div class="bg-site col-wh col-sm-12 head-pad hgh-40"> 
    <h5 class="m-t-n6" style="margin-bottom: 0px;">TEAM <?php if(isset($ac)) echo "hi";?></h5>    
        </div>
		<?php
 ...
 <table class="table table-striped table-bordered">
      ...
      <label class="form-check-label">
        <input type="checkbox" class="form-check-input cnames" name="cnames[]" value="<?php echo $rec['team_id'];?>" >
      </label>
    </div></td>     <td><?php echo $c;?></td>
                    <td><?php echo ucfirst($rec['name']);?></td>
                    <td><?php echo $rec['mobile'];?></td>
                    <td><?php echo $rec['designation'];?></td>
                   <td><a href="" ><img src="<?php echo base_url().'uploads/team_img/'.$rec['image']; ?>" style="height:50px;width:50px;"/></a></td>
                    <td><?php $s=$rec['team_status'];
					if($s==1){ ?>
					<a  href="<?php echo base_url();?>superadmin/team_ctrl/update_team/<?php echo $rec['team_id'];?>/<?php echo $rec['team_status'];?>/<?php echo $si;?>" class="badge badge-success">Active</a> 
					<?php }else{?>
					<a  href="<?php echo base_url();?>superadmin/team_ctrl/update_team/<?php echo $rec['team_id'];?>/<?php echo $rec['team_status'];?>/<?php echo $si;?>" class="badge badge-danger">Inactive</a>
					<?php } ?></td> 
                    <td><a title="Update" href="<?php echo base_url();?>superadmin/team_ctrl/update_team_member/<?php echo $rec['team_id'];?>/<?php echo $si;?>"><i class="fa fa-pencil-square-o text-primary" aria-hidden="true" ></i>
					</a>&nbsp;<a title="Delete" onclick="javascript:return confirm('Are you sure to delete');" href="<?php echo base_url();?>superadmin/team_ctrl/delete_team/<?php echo $rec['team_id'];?>/<?php echo $si;?>"><i class="fa fa-trash text-danger" aria-hidden="true" ></i></a> &nbsp;</td>
