...
<div class="col-sm-10">
 <?php $si=$this->uri->segment(4);?>
    <form method="post" action="<?php echo HTTP_BASE_PATH;?>admin/videos/search/<?php echo $si;?>" >
        <div class="col-sm-12">
    <div class="row m-t-15" style="margin-right: 0px;">


  <div class="bg-site col-wh col-sm-12 head-pad hgh-40"> 
    <h5 class="m-t-n6" style="margin-bottom: 0px;">Gallery Videos information<?php if(isset($ac)) echo "hi";?></h5>    
        </div>
	...
	<th>S.No</th>
      ...
      <label class="form-check-label">
        <input type="checkbox" class="form-check-input cnames" name="cnames[]" value="<?php echo $rec['id'];?>" >
                    ...
                    <td><p><?php echo ucfirst($rec['title']);?></p></td>
					<?php
					$url=$rec['url'];
					$ytarray=explode("/", $url);
					...
					$ytcode=$ytendarray[0];?>
                   <td><?php echo "<iframe width=\"420\" height=\"315\" src=\"http://www.youtube.com/embed/$ytcode\" frameborder=\"0\" allowfullscreen></iframe>";?></td>
				   <td><?php $s=$rec['status'];
					if($s==1){ ?>
					<a  href="<?php echo base_url();?>superadmin/upload_videos/update_status/<?php echo $rec['id'];?>/<?php echo $rec['status'];?>/<?php echo $si;?>" class="badge badge-success">Active</a> 
					<?php }else{?>
					<a  href="<?php echo base_url();?>superadmin/upload_videos/update_status/<?php echo $rec['id'];?>/<?php echo $rec['status'];?>/<?php echo $si;?>" class="badge badge-danger">Inactive</a>
					<?php } ?></td> 
                    <td><a title="Update" href="<?php echo base_url();?>admin/videos/update/<?php echo $rec['id'];?>/<?php echo $si;?>"><i class="fa fa-pencil-square-o text-primary" aria-hidden="true" ></i>
					</a>&nbsp;<a title="Delete" onclick="javascript:return confirm('Are you sure to delete');" href="<?php echo base_url();?>superadmin/upload_videos/delete_videos/<?php echo $rec['id'];?>/<?php echo $si;?>"><i class="fa fa-trash text-danger" aria-hidden="true" ></i></a> &nbsp;</td>
