...
class Diary extends CI_Controller {
    ...
    public function __construct() 
    {
        parent::__construct();
         ...
         $this->load->model('api/user_model','u');
		   $this->load->model('api/diary_model','d');
    }
	...
	public function get_specific_user_diary_data(){
		$response = array(); 
			$inputData=file_get_contents('php://input');
			  if(!empty($inputData)){
				if(isJson($inputData)){
                ...
                if(!empty($user_id))
                {
					$post_data=$this->d->get_user_diary_data($user_id);
				if(count($post_data)>0)
			 {
                            $response[CODE] = SUCCESS_CODE;
                            $response[MESSAGE] = 'Success';
		...
		public function get_specific_user_diary_data_between(){
		$response = array(); 
			$inputData=file_get_contents('php://input');
			  if(!empty($inputData)){
				if(isJson($inputData)){
				$reqdata=json_decode($inputData);
                $user_id=(isset($reqdata->user_id))?$reqdata->user_id:'';
				$from_date=(isset($reqdata->from_date))?$reqdata->from_date:'';
				$to_date=(isset($reqdata->to_date))?$reqdata->to_date:'';
				$error=0;
                $error_msg='';
                if(!empty($user_id))
                {
					$post_data=$this->d->get_user_diary_data_between($user_id,$from_date,$to_date);
   	    ...
   	    public function add_diary()
    {
        ...
        { 
            ...
            {	$reqdata=json_decode($inputData);
				...
				//$time=(isset($reqdata->time))?$reqdata->time:'';
				$user_id=(isset($reqdata->user_id))?$reqdata->user_id:'';
                $error=0;
                $error_msg='';
				if(empty($title))
                ...
                {
                    $error=1;
                    $error_msg.='Post date is required,';
                }
                if(empty($user_id))
                {
                    $error=1;
				...
				//date('H:i'),date("Y-m-d H") . ':00:00'
                if($error==0)
                {
					$date = date("Y-m-d", strtotime($date));
					//$time = date("H:i",strtotime($time)
					//$date = $date.$time.":00";
        $insert_data=array('title'=>ucfirst($title),'description'=>$description,'diary_date'=>$date,'written_by'=>$user_id,'written_on'=>$this->date);
