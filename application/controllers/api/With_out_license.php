...
class With_out_license extends CI_Controller {
	...
	$response = array(); 
		...
		//echo $id;exit;
		$accident_info = json_decode($this->Crud_model->licenseWithoutFetch($id));
		$html="";
		if($accident_info->code == SUCCESS_CODE)
		{
		 ...
		 //print_r($accident_info);exit;
		  $html.=$this->load->view('superadmin_view/without_license/without_license_view',$data,true);
					
					$this->m_pdf->pdf->WriteHTML($html);
					
					//download it.
					$filename = 'license_'.date('YmdHis').'.pdf'; 
					$pdfFilePath = "uploads/forms/".$filename;
					$res = $this->m_pdf->pdf->Output($pdfFilePath, "F");
					$data=array('user_id'=>$user_id,'pdf_number'=>"pdf3",'case_id'=>$id,'file_name'=>$filename,'date'=>$this->date);
					 $accident_id=$this->a->insert_accident("da_license_pdf_tbl",$data);
					if($res){
						//echo 'hi';exit;
					return $res = 1;
		}else{
   ...
   public function checkUserExistance($userid)
   {
    ...
    public function register_license_report()
    {
        $response=array();
        $inputData=file_get_contents('php://input');
        if(!empty($inputData))
        { 
            ...
            {	$reqdata=json_decode($inputData);
               ...
               $date=(isset($reqdata->date))?$reqdata->date:'';
                ...
                {
                    $error=1;
                    $error_msg.='submitted_to is required,';
                }
                if(empty($location))
                {
                    $error=1;
                ...
                $accident_id=$this->a->insert_accident("da_with_out_license_tbl",$data);
                 ...
                 if(!empty($accident_id)){ 
                   ...
                   // for($i=0;$i<$count;$i++)
					...
					// $path="uploads/images/";
					 ...
					 // $images_data[]=array(
                    // 'license_id'=>$accident_id,
                    // 'file_name'=>$picture,//'added_on'=>$this->date,group_id
                    // 'added_on'=>date('Y-m-d')
                         // ); 
						   // }
					// $res=$this->a->insert_damages('da_license_images_tbl',$images_data);
					 // }
					 if(!empty($picture)){
					  $count=count($picture);
                      $images_data=array();
			  ...
			  $res=$this->a->insert_damages('da_drivers_license_tbl',$damages_data);
				if($res)
						{
