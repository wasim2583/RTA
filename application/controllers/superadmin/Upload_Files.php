...
public function pagination1($base_url, $total_rows, $per_page){
	...
	public function gallery_information()
	{
		...
		$photos = $this->Base_model->get_all_photos();
		$this->data['row'] = $photos->result_array();
		...
		$config = $this->pagination1($base_url, $tr, $pp);
	 	...
	 	$row = $this->crud_model->common_fetch('files',$config['per_page'],$si,'id');
	 	// $this->data['row'] = $row->result_array();
