...
class Upload_photos_controller extends CI_Controller {
    ...
    public function photos(){
		$this->load->view('superadmin_view/photos/photos_upload_view');
	}
			...
			{   
	 			$multiple_image_count = count($_FILES['files']['name']);
   				if ($multiple_image_count > 0)
   				{
					...
					{
                 		$insert=  $this->crud_model->insert_batch('da_photos_tbl', $insert_image_array);
						if($insert)
						{
	 	...
	 	{//print_r($_POST);die;
	 		$arr=array(array('field'=>'name',
						...
						// array('field'=>'caption',
							// 'label'=>'',
							// 'rules'=>'required|regex_match[/^([a-zA-Z]+[a-zA-Z\-.\s]{2,60})+$/]',
							// 'errors'=>array('required'=>'Caption is required',
											// 'regex_match'=>'Slide caption allows only alphabets'),
						// )
					);
...
public function pagination1($base_url, $total_rows, $per_page){
 ...
 { 
	...
	$data['row']=$res->result_array	();//echo "<pre>";print_r($data);die;
	$this->load->view('superadmin_view/gallery_information_view',$data);
 }
